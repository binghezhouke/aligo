...
class SyncFolder(Core):
    ...
    def sync_folder(
            ...
            file_filter: Callable[[Union[str, BaseFile]],
                                  bool] = lambda x: False,
        ...
        if not os.path.exists(local_folder):
            ...
            return
        self.__sync_folder(local_folder, remote_folder, flag,
                           file_filter, ignore_content, follow_delete, drive_id)
    ...
    def __sync_all(
        ...
        if len(remote_files) != 0:
            self._auth.log.debug(
                f'本地不存在，下载云端文件 {len(remote_files)} {list(remote_files.keys())}')
            for remote_file in remote_files.values():
                if remote_file.type == 'file':
                    self.download_files([remote_file], local_folder)
                else:
                    self.download_folder(
                        remote_file.file_id, local_folder)  # type: ignore
    ...
    def __sync_remote(self, drive_id, local_files, remote_files,
        ...
        for f in list(remote_files):
            ...
            if f in local_files:
                ...
                if remote_size == os.path.getsize(local_file):
                    ...
                    if local_sha1 == remote_file.content_hash.lower():
                        self._auth.log.debug(
                            f'文件 {local_file} 已存在，且sha1值相同，跳过')
                        continue
                os.remove(local_file)
                self.download_files([remote_file], local_folder)
            else:
        ...
        if follow_delete and len(local_files) != 0:
            self._auth.log.debug(
                f'云端不存在，本地存在 删除 {len(local_files)} {list(local_files.keys())}')
    ...
    def __sync_local(self, drive_id, local_files, remote_files,
            ...
            if f in remote_files:
                ...
                if remote_file.type == 'folder':
                    ...
                    self._core_move_file_to_trash(
                        MoveFileToTrashRequest(
                            file_id=remote_file.file_id, drive_id=drive_id)
                    )
        ...
        if follow_delete and len(remote_files) != 0:
            self._auth.log.debug(
                f'本地不存在，云端存在 删除 {len(remote_files)} {list(remote_files.keys())}')
            self.batch_move_to_trash(
                [remote_file.file_id for remote_file in remote_files.values()])  # type: ignore
